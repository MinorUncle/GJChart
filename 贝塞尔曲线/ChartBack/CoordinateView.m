...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        testPath = [[UIBezierPath alloc]init];
        [testPath moveToPoint:CGPointZero];
        ////背景方块条
...
-(void)initShaperLayer{
    _coordinateLayer.strokeColor = [UIColor yellowColor].CGColor;
...
- (void)drawCoordinate {
    ...
    [_textView addTextWithPoint:zeroPoint text:@"0" textAlignment:TextAlignmentTop];
...
-(void)testDraw{
    count ++;
    int x = count % (int)(self.bounds.size.width);
    int y = (int)(count / (int)(self.bounds.size.height));
    [testPath addLineToPoint:CGPointMake(x, y)];
    [testPath stroke];
}
